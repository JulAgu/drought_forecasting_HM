...
def wrapper_ablation(expe_name, ablation_TS, ablation_tabular, ablation_attention):
    """
    A wrapper to repeat the train-evaluation cycle in an ablative way.
    """
    global master_dict
    # Taking adventage of the mutable nature of the dictionaries, not so elegant but practical.
    hyperparameters_dict["ablation_TS"] = ablation_TS
    hyperparameters_dict["ablation_tabular"] = ablation_tabular
    hyperparameters_dict["ablation_attention"] = ablation_attention
...
if __name__ == "__main__":
    # Paths
    ROOT_RESULTS = f"results/hm_ablation/"
    ROOT_TENSORBOARD = f"runs/hm_ablation/"
    ROOT_MODELS_WEIGHTS = f"models/hm_ablation/"
    ...
    wrapper_ablation("ablation_TS", True, False, False)
    wrapper_ablation("ablation_tabular", False, True, False)
    wrapper_ablation("ablation_attention", False, False, True)
    wrapper_ablation("ablation_tabular_attention", False, True, True)
